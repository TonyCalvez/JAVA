...
public class FX3DMonitor2 extends Application implements IMonitor {
	...
	// mapping entre un type d'entit� et le constructeur sans argument d'une
	// repr�sentation 3D
			...
			public void handle(ActionEvent arg0) {
				...
				if (!LogicalDateTime.EstBienStructuree(isoLogicalTime.getText())) {
					...
					if (dateToReach.compareTo(engine.SimulationDate()) < 0) {
						...
						alert.setHeaderText("La date saisie est inf�rieure � la date en cours");
						alert.setContentText("Date saisie = '" + isoLogicalTime.getText() + "'\n"
								+ "Date minimale � saisir = '" + engine.SimulationDate() + "'");
...
//			Logger.CloseAndReinit();
	...
	void newObjListener(ISimObject obj) {
		Logger.Detail(obj, "Monitor3D.newObjListener",
				obj.getName() + " configure son type pour une repr�sentation 3D.");
	...
	public void runPlanRT() {
		...
		if (selectNextScenario() != null) {
			...
			Logger.Detail(null, "runPlan", "d�but ex�cution");
			run(true);
		}
	...
	public void runPlan() {
		...
		Logger.Detail(null, "runPlan", "essai d'ex�cution");
		while (selectNextScenario() != null) {
			for (currentRepliqueNumber = 0; currentRepliqueNumber < getXpPlan()
				...
				setCurrentSeed(getXpPlan().getInitialSeed() + currentRepliqueNumber);// on prend le num�ro de r�plique
																						// comme germe initial de la
				// r�plique
				Logger.Detail(null, "runPlan", "d�but ex�cution");
				run(true);
				Logger.Detail(null, "runPlan", "fin ex�cution");
			}
		}
		...
		try {
			for (String s : ScenariiSettings.settings.scenarioInstanceClassNames) {
				...
				if (IScenarioInstance.class.isAssignableFrom(c)) {
					...
					Logger.Detail(null, "loadExperiencePlanFromSettings", "Sc�nario trouv� : " + scen.getName());
					scens.add(scen);
				} else {
